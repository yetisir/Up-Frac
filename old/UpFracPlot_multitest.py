...
def main(fileName, direction):
    ...
    plots = []
    # fig, axarray = plt.subplots(len(parameters.confiningStress), sharex=True, sharey=True)
    for i in range(len(parameters.confiningStress)):
        plots.append(UpFracPlot('test', '{0}({1}.{2})'.format(fileName, 0, parameters.confiningStress[i])))
        # plots[i].figure = fig
        # plots[i].axis = axarray[i]
        plots[i].setAxis(direction)
        plots[i].plotDemCurve(direction)
        #P.plotFemCurves('22')
    ...
    while True:
        ...
        for P in plots:
            ...
            currentNumFrames = P.plotCurrentFemCurve(h, direction)
        plt.show()
        if currentNumFrames == lastNumFrames:
            resultString = 'No new data found'
        else:
...
if __name__ =='__main__':
    ...
    # parameters = __import__('UDEC.modelData.'+fileName[0:]+'_modelData', globals(), locals(), ['*'])
    parameters = importlib.import_module('UDEC.modelData.'+fileName[0:]+'_modelData')
